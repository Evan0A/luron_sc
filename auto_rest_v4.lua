...
script_mode = "ANY" -- support "ANY" or "ROTATION"
...
api_player = ""
api_use = ""
access_url = "https://pastebin.com/raw/NYL7ErKz?t="..os.time()
...
function webhookRest(nameBot, from)
    if use_webhook and not whrestdone then 
        ...
        wh.embed1.author.name = "NEXORA"
        wh.embed1.author.icon_url = "https://i.imgur.com/DAUoU3G.jpeg"
        wh.embed1.author.url = "https://discord.gg/6Y2ZZFXk"
        if not all_bots_send_webhook and getBot().index == 1 then 
            wh:send()
            whrestdone = true
...
function webhookRecon(nameBot, from)
    if use_webhook and whrestdone then 
        ...
        wh.embed1.use = true
        wh.embed1.title = string.format("[%s]RECONNECTED", emoji)
...
function getCaptain()
    ...
    if getBot().index == captain then 
        ...
        getBot().custom_status = string.format("Following captain(%d) rest time", captain)
    end
end
    ...
    for _, users in pairs(data.access_list) do
        if users.name == myUsername then
            ...
            if users.expired == "never" then 
                ...
                webhookAny("License approved, with name: "..users.name)
                print(reason)
                return true 
            else
...
function restSpecificMod()
    if auto_rest_specific_mod then 
        if getBot().index == captain then 
            for _, names in pairs(mods_list) do 
                for _, namis in pairs(specific_mod_list) do 
                    if names == namis then 
    ...
    if script_mode:upper() == "ANY" or script_mode == "" then 
        ...
        while true do 
            ...
            if nowMin % any_check_delay == 0 and nowMin ~= lastCheckedMinute then
                restAll()
                print("loop any, cpu: "..get_cpu_usage())
            end
        end
