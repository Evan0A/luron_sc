...
function clearfarm(world)
    botInfo("Clearing farm blocks")
    for _, tiles in pairs(getBot():getWorld():getTiles()) do 
        if tiles.fg == itmId then 
            getBot():findPath(tiles.x, tiles.y)
            while tiles.fg == itmId do 
                getBot():hit(tiles.x, tiles.y)
                sleep(delayPunch)
                reconnect(world, doorFarm, tiles.x, tiles.y)
            end
        end
    end
end
...
function harvest(world)
    ...
    elseif not separatePlant and not dontPlant then
        ...
        for _,tile in pairs(getBot():getWorld():getTiles()) do
            for _, obj in pairs(getBot():getWorld():getObjects()) do
                if obj.id == itmId and getBot():getInventory():getItemCount(itmId) <= 170 then
                    getBot():findPath(math.floor(obj.x // 32), math.floor(obj.y // 32))
                    sleep(600)
                end
            end
