...
function warps(world, id)
    ...
    nuked = false
    stuck = false
    if not bots:isInWorld(world) then
        bots:leaveWorld()
        sleep(2000)
        ...
        while not bots:isInWorld(world) and not nuked do
            while client.status ~= BotStatus.online do
                sleep(5000)
            end
            bots:warp(id == '' and world or world .. ('|' .. id))
            listenEvents(math.floor(delayWarp / 1000))
            sleep(delayWarp)
        end
        removeEvent(Event.variantlist)
    end
    if bots:isInWorld(world) and id ~= '' and getTile(client.x, client.y).fg == 6 then
        local count = 0
...
function botInfo(info)
    ...
    webhook.embed1:addField("<:robots:1037182734067576842> Bot Captcha", getBot().captcha, true)
