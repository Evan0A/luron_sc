...
customStatus = { 
	...
	takeItem = "Searching item",
	joinRandom = "Clearing world history"
}
webhookEmoji = {
    bot = ":robot:", 
    storage = ":package:", 
...
function rangkum(teks, panjang_maks)
    -- panjang_maks adalah jumlah karakter sebelum ditambah ".."
    if #teks > panjang_maks then
        return string.sub(teks, 1, panjang_maks) .. ".."
    else
        return teks
    end
end
    ...
    if getBot().index == bot_webhook then 
        ...
        for _, bot in pairs(getBots()) do 
            if bot.index <= max_show then 
                botdata = botdata.."["..bot.index.."]".." ||"..rangkum(bot.name, 4).."||("..bot.level..") | "..getStatus(bot.status)..status_map[bot.status].."["..bot:getPing().."] | ||"..rangkum(bot:getWorld().name, 5).."||\n"
            end 
...
function getStatus(stat)
    ...
    if stat == 1 then
        return online
    elseif stat == 3 or stat == 4 then
        return webhookEmoji.banned
    else
        return offline
    end
end
function storageAdd(world, id, count)
    ...
    for _, stg in pairs(storage) do 
        if stg.world == world then 
            foundworld = true 
            for _, item in pairs(stg.item) do 
                if item.id == id then 
                    item.count = count 
                    added = true
                end 
            ...
            if not added then 
                table.insert(stg.item, {id = id, count = count})
            end 
        end 
    end 
    if not foundworld then 
        table.insert(storage, {world = world, item = {{id = id, count = count}}}) 
    end 
end
...
function storageCount(id)
    local count = 0
    for _, stg in ipairs(storage) do 
        for item in ipairs(stg.item) do 
            if item.id == id then 
...
function storageTotal()
    ...
    for _, data in ipairs(storage) do
        for _, itemData in ipairs(data.item) do
            if totals[itemData.id] == nil then
                totals[itemData.id] = itemData.count
            else
                totals[itemData.id] = totals[itemData.id] + itemData.count
            end
        end
    end
    local result = {}
...
function dropItem(itemID)
    ...
    while true do 
        ...
        scanStorage()
        if bot:isInWorld(world) and itemCount(blockID) < 200 and gscanFloat(blockID) > 0 then 
            for _, obj in pairs(getObjects()) do 
                if obj.id == blockID then 
                    ...
                    sleep(200)
                    local bitem = itemCount(blockID)
                    while itemCount(blockID) == bitem do
                        reconnect(world, door_storage, objX, objY)
...
function takeItem(itemID, amount)
    ...
    bot.object_collect_delay = 100
    if amount == nil then 
        amount = storageCount(itemID)
    end
    local found = false
    ::getItem::
    while storageCount(itemID) >= amount and not found do 
        if #storage == 0 then 
            break 
        end
        ...
        if name then 
            ...
            while itemCount(itemID) > amount do
                ...
                getBot():drop(itemID, itemCount(itemID) - amount)
                sleep(2000)
                found = true
                scanStorage()
                return
    ...
    if not found then 
        for i = 1, storageMaxFind do
            warps(world_storage[i], door_storage) 
            reconnect(world_storage[i], door_storage)
            scanStorage() 
            if storageCount(itemID) >= amount then 
                found = true
                goto getItem
            end 
        end 
        if not found then
            local name = world_storage[1]
            warps(name, door_storage)
            callAlert(getBot().name.. " looking for item "..getInfo(itemID).name.."(x"..amount..") drop it at world: "..name)
            reconnect(name, door_storage)
            while not found and itemCount(itemID) < amount do 
                reconnect(name, door_storage)
                for _, obj in pairs(getObjects()) do 
                    if obj.id == id then
                        local objX = obj.x//32 
                        ...
                        sleep(500)
                        reconnect(name, door_storage, objX, objY)
                        local bitem = itemCount(itemID)
                        while itemCount(itemID) == bitem do
                            getBot():collectObject(obj.oid, 2)
                            sleep(1000)
                            reconnect(name, door_storage, objX, objY)
                        end 
...
function joinRandom() 
	if warp_random_world then 
	    ...
	    addEvent(Event.variantlist, limit)
	    getBot().custom_status = customStatus.joinRandom
...
function verify() 
    local function GTverify(data)
        if not getBot():isInWorld(verify_world:upper()) then 
            warps(verify_world:upper(), "")
        end 
        while not worldNuked do
            reconnect(verify_world:upper(), "")
            for _, plr in pairs(getPlayers()) do  
                for user in pairs(data.accessList) do 
        ...
        if worldNuked then 
            callAlert("world verify is nuked, bot stopped")
            getBot():stopScript() 
        end 
    end
