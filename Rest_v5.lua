...
real_link = "https://raw.githubusercontent.com/Evan0A/Nuron_access/refs/heads/main/Rest_Script"
...
function getHttp(url)
    if not auto_rest_many_mods and  not auto_rest_specific_mod and not auto_rest_player and not url.find(real_link) then 
        return false 
    end 
...
function webhookRest(nameBot, from)
    if use_webhook then 
        ...
        elseif from  == 3 then 
            ...
            lastrestid = 3
        elseif from  == 4 then 
            extra_info = string.format("Rest until Ban-rate lower, ban-rate: %f", banrateApi)
            wh_banrate = banrate
            lastrestid = 4
        elseif from  == 5 then 
            extra_info = ""
...
function disconnectBot()
    ...
    if getBot().status == 1 then 
        for _, ib in pairs(bot_indexs) do 
...
function restManyMods()
    if auto_rest_many_mods then 
        if getBot().index == captain then 
            ...
            while isIn(captainStatus, getBot(captain).custom_status) and getBot(captain).custom_status ~= "Online" and getBot(captain):isRunningScript() and getBot().custom_status ~= "stopped" do 
                disconnectBot()
                sleep(delay_many_mods * 60 * 1000)
                if script_mode == "ROTATION" then 
                    rotation.enabled = false
